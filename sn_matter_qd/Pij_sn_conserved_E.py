import numpy as np
import math
from numpy import array, sqrt, trace
...
# data_ea = np.loadtxt('Earth_profile.txt', delimiter=',', skiprows=8)
# ne = data_ea[:,1] * cm3_to_eV3 * Na #eV^3
# r_data = data_ea[:,0] #km
...
#uncomment next line to save compiled compatible to OS
# @cc.export('Pij_SN_surface_E_conserved', 'f8(f8, f8, f8, string, string, string)')
def Pij_SN_surface_E(n, E, g_exp, mix, ij, nu_nubar):
    gamma = 10**g_exp
    gamma = gamma * (E*1e6)**n
    ...
    def lindbladian(E, vcc0):
        ...
        # Gamma_vec = gamma,gamma,0,gamma/4,gamma/4,gamma/4,gamma/4,0

        def Dm_function(a_vec, gamma):
...
# p = Pij_SN_surface_E(0, 61, -20, 'NH', '11', 'nue')

# ti = time()
# # Pij_SN_surface_E_vec = np.vectorize(Pij_SN_surface_E)
# p = Pij_SN_surface_E(0, 61, -20, 'NH', '11', 'nue')
# print(time()-ti)
# print(p)
...
# ti = time()
# Pij_SN_surface_E_vec = np.vectorize(Pij_SN_surface_E)
# p = Pij_SN_surface_E_vec(0, 10, 1e-30, 'NH', '12', 'nuebar')
# print(time()-ti)
